...
### Current state
Version 0.3.0, which hardened and polished what 0.2.0 delivered, without opening a new bounded context. It brought the responsive interface with two breakpoints, the graph assembled from attribute switches, with a group that holds on click and that answers touch, the plan quota actually enforced instead of merely declared, the reading surface of notes following the metrics of the default Obsidian theme, and the fix of consistency defects between the Discovery projection and the content of the notes. Before it, 0.2.0 delivered the six bounded contexts, the whole infrastructure in CDK, the interface wired to the real API and the MCP connector with reading and writing, authenticated by the CIMD proxy in front of Cognito; and 0.1.0 delivered the canonical documentation, the navigable frontend prototype over the seed and the MCP authentication spike. The environment goes up and comes down through the scripts in `deploy-aws/`, and deployment to AWS happens with step-by-step supervision by the user.
