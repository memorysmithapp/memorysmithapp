...
### Current state
Version 0.8.0, where the screens of a notebook are redrawn the way 0.7.0 redrew the ones around it, and where **the note is the one thing left exactly as it was**. **Every screen of a notebook opens under one fixed bar** holding the trail, and no heading repeats what the trail says; the actions of a note moved into it, and the history of a note, a fold after its last line that a reader of a long note never reached, opens from it as a modal on a computer and a sheet on a phone. **The notebook is shown as what it is made of**: the Notebook Context is three cards of one weight — Guidance, Templates and Folders — the folders got the page of their own their address had been answering not found since 0.7.0, the sidebar lists the same parts with the graph under them and tells what is open by weight alone, and a folder reads as its description, its Template as a card and two counted groups. On a phone the navigation stops being a drawer from the left and **rises from the foot of the screen**, a row of icons that stays put over the tree. Deleting a Guidance or a Template names what it deletes and asks in place, the search opens its results as a menu over the reading with the term marked in orange instead of blue, and the graph says which value it is holding. Three defects were paid along the way: the space panel grouped a fourth notebook as *Mais 1 caderno* in the room of the line that would name it, an invalid Mermaid diagram left its error at the foot of the whole application, and a deep trail on a phone cut off the folder being read, where it now collapses its middle into *…* first. Before it, 0.7.0, where the interface is drawn to a design somebody approved instead of one that accumulated, and where a person finally sees what fills their space. **The controls speak one vocabulary**: blue is spent on the main action, a link and focus, and everything chosen is Tinta — the open note, a tab, a theme, a switch — where it used to wear the colour of an action; fields and buttons share one radius and one focus ring, a modal has its × and its actions on the right, and the dark theme draws in the colours of the dark version of the brand book, with the one red the book does not have said to be ours. On that vocabulary Home, Transfers, About, the profile and the change of password were redrawn, each from its approved screen and each opened on a computer and on a phone before it was called done. **Home stops counting links and starts counting space**: the four counts about links left the door for the space of the subscription, the total against the quota split by what fills it and by the notebook that holds it, answered from counters kept with the total so reading it opens no note and no file. A notebook is **exported and deleted from its card**, by its owner and without an agent; an export kept in Transfers is **imported from where it is kept**, its bytes copied rather than travelling through the device, and an import that ended is deleted from Transfers as an export is. The agent gained `check_notebook`, which lists the names a notebook links to that no note carries and the notes nothing links to, and names what a link that looks broken most likely meant — a pending link is fine on purpose, one that almost reaches a note is not — and both skills end with that sweep. **The screens follow what somebody else writes**: a note an agent writes through the connector used to stay as it was first read until a reload, and every screen reads again every half minute while visible, never under a write that is waiting, saying so when a note changed under its reader. Two things left: the Root of the folders, a node, a crumb and a page repeating what the page of the notebook lists; and the AWS pipeline, since GitHub Actions delivers both environments through roles a run assumes with no key stored. And one defect of 0.6.0 was paid: deleting a notebook left every file it kept in the store and on the bill, and the cleanup now destroys them before the tree, while `recount-storage` stops charging for the ones left behind before this version. Before it, 0.6.2, a correction cycle of what the product says about what it holds. Discovery never heard, in AWS, that a notebook **keeps a file**: the rule feeding the projection carried neither file event, so every embed of a file that exists was a pending link, and a notebook with one real pending link and 49 pictures showed 50. Routing the two events was half of it — the graph decided *file or pending* at the instant a note was written, and a file is kept and deleted without rewriting any note, so whether a target is a file is read now instead of frozen then, and a rebuild learns the files kept before this version. **Footnotes** arrived the other way round: nobody had declared them and the parser rendered them anyway, so following one opened the whole application in a new tab, the notes were headed in English whatever the reader spoke, and each definition was read as a link reference definition — `[^1]: Wikipedia.` making every footnote a pending link named after the first word of its source, which is the one thing §8 forbids an implementation to derive from a form it does not describe. They are §7.12 now, display only, with three conformance cases and a place in both skills. Around the two, four things the screen said wrongly: the links of a note dropped the `kind` the contract declares, so an embedded file and a pending link answered the same object and the connector called every picture pending; the listing of a whole notebook answered in the order the notes were created, so a note the agent moved with `reorder_note` stayed where it was born; a sentence with two counts pluralised neither and read "1 links"; the page of every Template wore the header of a single folder; and the welcome page promised backlinks no screen shows and an export of Markdown files in folders that is one archive. **Every fix was opened, clicked and read on staging before it was called done**, and the two the suite could not have caught — a rule of the bus and a route dropping a field — are each held by a case now. Before it, 0.6.1, a correction cycle of the one page that tells a person where the agent lives. The welcome surface published the connector as its host, `https://mcp.memorysmith.app`, where no server answers, and the endpoint is `/mcp`; every other consumer of the connector appended the path, and the page a person reads was the one that did not. Its steps sent a person to screens the two clients no longer have — Claude moved connectors to Customize, ChatGPT calls them plugins behind a developer mode offered only in the browser — so they were reread in both and rewritten with the answer to each field. And the page now opens on what the product is, a person and their agents writing the same notebooks, where it used to close its first paragraph on an unnamed *us*. Before it, 0.6.0, where a notebook stops being only notes and the interface stops being only a place to read them. A notebook **keeps files** now — bytes with a name, a description, a type, tags and a path — and a note shows one by writing `![[name]]`, which is the same address a note is reached by: the name, never an identifier and never a path, so moving a file breaks nothing and an imported picture is found by the note that always found it. Sixteen types are accepted and the declared type is checked against the bytes, because a page kept under the type of an image is harmless until something serves it as what it is. The interface **writes**: a note is edited in the Markdown it was typed in, and the write says what it will do to every link before it is confirmed; a person edits their own name, their picture and their password; the first sign-in lands somewhere that says what the product is and where the agent lives. Portability grew into two jobs that finish: an export is built by a worker and kept in Transfers until it is deleted, an import reads its document in the browser before a byte is uploaded, and both choose **what travels**, species by species, the history and the files included. Deleting became definitive on purpose, and takes the trail of the notebook with it, which is what makes an export the one place a history survives.

The second half of the cycle is the part nobody plans: **the door that had just been built was opened, and it answered wrong five times.** A notebook that kept a file could not be exported at all, because the archive validated its trail against a hand-copied list of subjects that `FILE` never entered; the ceiling the connector declared, 8 MB, was unreachable, since the request carrying the bytes stops at 6 MB and base64 costs a third on top; the card of a document offered two verbs that were one link, so opening a PDF downloaded it; a dimension after the pipe was read and then dropped; and a **link** to a file drew the file, which made `[[picture]]` and `![[picture]]` the same thing. Each of the five was found by opening the screen and using it, none by the suite, and each one left behind the case that was missing — including the two regressions the fixes themselves shipped, one of them a cache key answering two shapes. What the cycle leaves written down is that number: a green suite proved nothing about any of them.

Before it, 0.5.7, where the specification the product implements stops describing itself in tiers, and the product finds out what it was reading off that description. The MemorySmith Markdown Specification released v0.4.0 and removed three fields: the ring each form belonged to, the flag saying whether a form was read, and the list of specifications the profile was built out of, which became a list of **sources a form is credited to**. The last one is the smallest change and the largest correction, because an implementation is no longer asked to support a source in full — a requirement nobody can check — but to support the notation the document lists, which the suite settles. Obsidian is named for the first time, for the seven notation families that came from it, and the precedence runs the other way for it than for the other two: the profile defers to CommonMark and GFM and governs over a vault editor. Its entry carries no version, because it publishes documentation rather than a specification, and read as required that is how the skill served an agent `Obsidian undefined`. Unlike the 0.3.0 break this one was caught by the compiler, at the seam, before a test ran. What the compiler could not catch is that two decisions of this repository had been reading themselves off those fields — which forms the reading-surface expectations and the demonstration vaults are asked for, and what the product deliberately does not read — so both are now written here, asserted against the profile, and a form a later version adds fails the build until somebody classifies it. What the product does with `#assunto` did not change; the sentence saying so is ours now. Around the upgrade, four debts of the reading surface, and the two the new sections name were real. A sentence with two prices in it lost its middle: the dollar rule had the outside edge of each prohibition and not the inside one, so `R$100 e o frete R$200` came out with `100 e o frete R` typeset as mathematics — an ordinary sentence in pt-BR, where a price is written `R$`. An embed written in a table cell did not merely fail to expand: the split that decides expansion runs on the raw string, so it cut the row in half, emptied the cell and left the closing pipe as a paragraph. The third is the one addition of the cycle the product answered with silence, which is the only answer §7.10 forbids: opening a note fetches the image whose destination names a host, and now the note says so, naming the hosts, where a person reads it. And the fourth is not the profile's at all: a fenced block that named its language rendered as unstyled monospace, and it is highlighted now — under the four rules the profile does impose on anything that close to a note, the sharpest of which is that the tokens arrive as elements, because a highlighter returning HTML to be injected takes the body of a note as its input. Before it, 0.5.5, where the one thing the product sends stops being written by somebody else. The invitation and the recovery code carried the provider's factory text, in English whatever the account spoke, and they are the screen before the sign-in page that is fully dressed in the brand. They now say what they are, in Portuguese and in English. The sender is still the provider's, because only that half was ever blocked on AWS. Before it, 0.5.4, where the write finally says what happened to it. Ticking a box gave no sign of anything, and the only way to find out was a reload — which, inside the two-second grouping window, was itself what destroyed the write. The status left the body of the note, where it scrolled away with the text, for the frame of the screen, and it now says the whole life of a write instead of only its failures. Before it, 0.5.3, two ways the screen showed something that was not there: a cache nobody told to forget after the application's own write, so coming back to a note showed the state from before the edit; and a wikilink inside transcluded content rendered as raw text, brackets and all. The staleness policy of the interface is now written down instead of being a comment saying somebody else would bring one. Before it, 0.5.2, the third and last layer of the same feature, where each fix uncovered the next: the task box did not toggle at all, then the write it unblocked was refused at validation, and then the second write of a checklist conflicted with the first. The client sent the revision it loaded with and never adopted the one its own write produced — and could not have, because of the three writes of a Content Slot only the guidance answered a revision. All three answer it now, and a conflict on a task box finally means what it says: somebody else wrote. Before it, 0.5.1, a correction cycle of a single issue, and of the kind worth naming: the interface had retyped an API response by hand and got it wrong, so it echoed a whole content reference where the API expects a version and every task-box write was refused at validation — while the screen announced a conflict for it. It survived a whole release because the note was the one surface nobody had exercised: the box itself had not worked since 0.4.0, and fixing it in 0.5.0 is what exposed the layer underneath. The origin was fixed with the symptom: a response is now typed by the DTO the API publishes, so the next divergence is a build error rather than a screen that fails. Before it, 0.5.0, the cycle in which the product stopped declaring its own Markdown notation and started **implementing a published specification**: the MemorySmith Markdown Specification, versioned outside this repository, pinned in one place, and proved by its own conformance suite against the two sanctioned extractors and against the reading surface. That single change paid for itself five times over the cycle, because a suite you did not write finds what a test you wrote does not: a list of one item classified as an enum, a task box that had not worked on any note since it shipped, seven declared notations the interface silently did not implement, a price rendered as a formula, and a slug computed two different ways so that a real link showed as missing. Around it, the vault ring was completed on the page — marked text, comments, block identifiers and their embeds, mathematics — and the two silences were made explicit: raw HTML is not rendered, as a security boundary, and superscript and subscript have no notation here. The frontmatter gained a reserved vocabulary, dates gained an interval and the query language gained its first operator. The interface paid four debts: a session that died without saying so, a wait that showed nothing, a sign-out that asked for a click deciding nothing, and the first write the web interface makes. And two vaults, one in en-US and one in pt-BR, became the only place the profile can be read rather than proved. Before it, 0.4.1, a correction cycle of a single issue: the Vault Context prints the identifier of every folder, which was returned once by `create_folder` and by nothing else, so a session that had not built the structure could read the whole vault and write in none of it (RN-AGT-020). Before it, 0.4.0, the cycle that closed the distance between what the documents promised and what the product does, and that opened a door for whoever uses it: the documents stopped describing screens that were never built, the roadmap and the risks left `docs/` and became issues, the whole repository was translated to en-US, and on the product the transclusion, the clickable task list and the skills arrived. Before it, 0.3.0 hardened what 0.2.0 delivered, with the responsive interface, the graph with facets, the plan quota actually enforced and the reading surface following the metrics of the default theme of the desktop vault editors; 0.2.0 delivered the six bounded contexts, the whole infrastructure in CDK, the interface wired to the real API and the MCP connector authenticated by the CIMD proxy in front of Cognito; and 0.1.0 delivered the canonical documentation, the navigable prototype over the seed and the MCP authentication spike. Each environment is delivered by a command, `deliver`, which builds, deploys in the order the environment imposes and closes with the smoke, and GitHub Actions runs it: staging when somebody starts it on a branch, production on every merge to `main` that touches what is deployed, through roles a run assumes with no key stored anywhere. Bringing an account up happens once, with step-by-step supervision by the user.
...
└── memorysmith-infra/       # all CDK: stacks, constructs, IAM policies, delivery roles
```
...
| 6 | **No path alters an entry of the audit trail, and exactly ONE principal removes one**, the purge, and only as part of a deletion. It is IAM that says so, not discipline | §12.2 |
| 7 | **Every domain operation that changes state takes an `Authorship`.** There is no anonymous mutation | §12.1 |
| 8 | **Deleting is definitive and invalidity is inherited**: one write on the unit deleted, everything under it invalid in the same instant, and a background purge that destroys the content, the items and the projections. Nothing restores. Exactly ONE principal may destroy a revision, the purge worker, and it reaches the store through a port of its own | §12.4 |
...
**Inviolable:** the version bump reaches `main` only through a PR, never through a direct push. The tag is written by `publish-release`, the last step of the production workflow, **after** the merge and once production serves the version, and points at the merged commit; **nobody tags by hand**. What holds the guarantee is the command: it refuses a version that disagrees anywhere, it annotates the tag in two steps, it takes the notes from the section of that version, and it refuses a tag that already points at another commit, because a version names one commit. A change that alters nothing deployable, such as documentation and repository governance, **does not cut a version**: it waits in `[Unreleased]` for the next cycle, which takes the whole accumulation. The ten-step flow is in `development-process.md` §9.1, and what cuts a version, in §9.
...
Every PR description contains three sections: a **Summary of changes**, a **Staging validation** and an **AI productivity analysis**. Delivery is a command somebody runs, so the staging section is written by whoever ran it: the version staging served and what was exercised on it, or the sentence saying nothing of the branch ran there. Never block a merge on it. The format of the three, and how to fill in each field of the last, are in `development-process.md` §8.
