...
- **A way in for whoever uses the product.** Two issue forms, one for **usage feedback** and one for **scope proposals**, plus the `/triagem-feedback` command that groups open feedback by real friction instead of by request. The feedback form asks for the suggested solution last, and explicitly optional, because the description of the friction outlives the proposed fix.
