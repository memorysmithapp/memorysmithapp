...
- **RN-DSC-046:** A wikilink target that does not resolve to exactly one note is addressed as a **target**, under `/notebooks/:notebookId/links/`, and never as a note. It renders the choice when several notes answer and the pending state when none does, lists every candidate with the folder it lives in, and says whether the target matched a name or an alias — because the two are not equally durable (RN-DSC-053). It is notebook-wide, because resolution is.
