...
| Transfers | Every export and every import of whoever is signed in, with what each one is doing or what it produced: an icon beside the user menu, in the place a browser puts its downloads and never as a notification bell, and a page with the history. An export is downloaded again from here, through a link issued at that moment, imported from here as a new notebook without passing through the device, and deleted from here, which destroys its bytes. A running import is cancelled from here, which takes it back down whole (RN-PRT-018), and a transfer that failed says why. The space the kept exports occupy is stated at the top, because it counts towards the quota (RN-PRT-019, RN-PRT-020, RN-SUB-021) |
