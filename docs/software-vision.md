...
| Import a notebook | A page of its own, in three steps: the file, which the **browser reads on the spot** — unzipping it and validating the document against the same format the server does, so what it carries is seen before a byte is uploaded; the review, where the name comes from the document and is checked against the notebooks of the subscription as it is typed, and where what to import is chosen — everything, only the structure, or items picked in a tree whose checkbox takes a whole branch and whose row control takes the node alone; and the progress, which can be left and come back to. It ends as the report of RN-PRT-015 with the notebook to open, or as the reason it was refused, in the language of the interface, with nothing kept (RN-PRT-017, RN-PRT-018) |
| Notebook → Export | Starts the export of the whole notebook as one `.notebook` document, in the format of §12, and says where to follow it. The download starts by itself when the job ends while the person is still on that page, so the common case of a small notebook stays one click (RN-PRT-019) |
