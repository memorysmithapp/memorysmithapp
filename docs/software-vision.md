...
2. **Curation.** People review what came in and drive the corrections, and the interface is where they read: the note and the structure as the agent receives them, the distribution of the notebook on its graph, which colours and filters by the facets (§10.3), and every link of a note, which says when it reaches nothing yet.
...
- **RN-SUB-023:** The files a notebook keeps are current content of the subscription and count against its quota (RN-SUB-021). An upload with no room is refused before the bytes are stored, and deleting a file releases them.
- **RN-SUB-024:** What fills the space of the subscription is answered beside its total, from counters kept with the total and never by reading content: the total and the quota, the same the session reports (RN-SUB-021); the total **split by kind**, with how many and how many bytes of each — notes, files, kept exports, and the `Guidance` and the `Template`s together as *others*; how many **notebooks, folders and revisions** the subscription holds, a revision being counted and never charged; and **a line per notebook**, largest first, with its bytes and how many notes, folders, files and kept exports it holds. The totals are about the subscription and are the same for every role; the lines are only the notebooks the requester may read (RN-SUB-004), so for anyone but the owner they may add up to less than the total. A deleted notebook stays counted, and listed, until its purge ends, when its bytes leave the total (RN-KNW-047). The counters are rebuilt by the same recount that rebuilds the total. No MCP tool answers it.
...
- **RN-ACC-019:** An account is shown **what the product is** once, on the first sign-in that reaches the dashboard, and never again on its own; from then on the same surface is reached from the user menu. It carries what a notebook is and what each screen of the interface gives. The date is recorded on the account and not in the browser, because being welcomed happened to the person and not to a device.
- **RN-ACC-020:** The welcome surface states **the address of the connector of its environment** and how to add it to a client, because nothing else in the interface does. An agent writing in the notebooks is what this product is, and until it is pointed at one the account is a reading tool over an empty shelf.
...
-- the PICTURE is not here: it is data of the person inside one subscription (RN-ACC-022)
...
- **RN-KNW-043:** **A folder issues increasing whole numbers**, starting at 1, one per request. A number is never issued twice and never reused, not even after the note that carried it is deleted, and a number issued and never used leaves a gap. The product knows the number and nothing else: no name is checked against it, and whether a folder numbers its notes, with which prefix and which padding, is a convention the Guidance states (PP4). Requests on one folder at once are served one after the other and none of them fails. Issuing a number is not an audited event: the number enters the record when a note names itself with it, and that write is.
...
- **RN-KNW-033:** **Deleting a notebook is definitive**, and takes everything it holds with it: the notebook starts answering `404` in every context at once, and so does every folder, note, Template, Guidance and file under it (RN-KNW-046), all of which are purged afterwards (RN-KNW-047). The operation belongs to the notebook administration role, like renaming. The name of the notebook is free in the subscription the instant it is deleted, because the notebook that held it is never coming back to claim it.
...
- **RN-KNW-048:** A notebook keeps **files** beside its notes: bytes with a **name**, a **description**, a **type**, **tags** and a **path**. The name is what a note addresses with `![[name]]`, wherever the file sits; the path organises it, is written like a path of a filesystem and exists because a file was written into it — there is no folder to create and none to delete when the last file leaves. Neither the path nor the extension of the name takes part in identity, so moving a file breaks no note.
- **RN-KNW-049:** A notebook keeps **one file of each name**, and a name it already keeps is refused naming the file that holds it. A link can offer a choice between two notes of one name; an **embed** cannot — it expands one thing or nothing — so a second file of one name would make `![[name]]` undrawable.
- **RN-KNW-050:** The types a notebook accepts are a **closed list**: `image/png`, `image/jpeg`, `image/webp`, `image/gif`, `image/svg+xml`, `application/pdf`, `audio/mpeg`, `audio/wav`, `audio/ogg`, `audio/webm`, `video/mp4`, `video/webm`, `video/quicktime` and the three Office documents. Anything else is refused before a byte is stored, with the list in the refusal. **The declared type is checked against the bytes**, and a type they do not support is refused naming both: the extension of a name decides nothing, so the only thing that can establish what a file is is the file. What is served carries the stored type. **What a browser shows is served `inline` where it is opened and as an attachment where it is saved**, which is what lets a card offer to open a PDF and mean it; what no browser shows — the three Office documents — is served as an attachment under both, and the surface offers the one verb it can keep (#171).
- **RN-KNW-051:** Deleting a file is **definitive**, its name is free at once, its bytes are destroyed by the purge (RN-KNW-047), and every note that referenced it renders the way a pending link does from that instant.
- **RN-KNW-052:** A note is **written from the interface**, in the Markdown it was typed in — the frontmatter included, the wikilinks unresolved, the embeds unexpanded — by somebody whose effective role in that notebook writes. The write carries the revision the screen loaded with, and a divergence means **somebody else wrote**. Editing the body is how a note is renamed (RN-KNW-035) and how it can be left with **no name** (RN-KNW-036); neither is refused, and the screen states what the write will do — the name it will carry, how many links go pending and how many resolve — **before** it is confirmed, and offers to keep the old name as an alias.
- **RN-KNW-053:** A file kept through the connector travels **inline, base64, up to 4 MB**; above it nothing is stored and the refusal states the ceiling. The number is the **transport** and not a preference: the request carrying the bytes is a synchronous invocation that stops at 6 MB, base64 costs a third on top of the file, and what travels beside them — the name, the description, the tags, the path — takes the rest. A ceiling the door cannot reach is a promise and not a limit: the file that exceeds it is refused by the platform, with a status that names no ceiling and no unit, before the message explaining the limit can run. Keeping more than this means taking the bytes out of the request, the way an import already does (RN-PRT-014), which is a surface for a person rather than for an agent — an agent filling the field pays for every byte twice, in the call and in what it had to write to make the call.
...
| `next_number` | `(notebook, folder)` | Issues the next number of a folder, for the records a Guidance numbers, such as `EV-00042`; a write, and not idempotent (RN-KNW-043, RN-AGT-036) |
| `list_notes` | `(notebook, folder?, limit?, cursor?)` | The index of notes in the defined order, each as identifier, name, folder and position, in pages continued with a cursor (RN-AGT-031) |
| `read_note` | `(notebook, note, asOf?)` | The full Markdown and the current revision, the trail of its folder and where each of its links goes (RN-AGT-033, RN-AGT-034); with `asOf`, the revision in force on that date, without the links |
...
| `search_notes` | `(notebook, query)` | Literal search over the text of the notebook, with fields and operators (§10.2), each hit with its folder |
| `related_notes` | `(notebook, note, depth?)` | A dependency tree through the link graph, each note with its folder |
| `backlinks` | `(notebook, note)` | Who points at this note |
| `check_notebook` | `(notebook)` | What the notebook left pending, the links that look broken with the name each most likely meant, and the orphan notes: the sweep before a piece of work is handed over (RN-AGT-039) |
...
3. **Trabalhos/** `01J2Q4X8V6ZK9M3B7C5D1F0GHW`: Relatórios emitidos. (0 notes here, 5 in subfolders)
...
- **The note count comes along**, split for a folder that has subfolders into the notes it holds directly and the notes below it (RN-AGT-035). The agent knows where the mass is before asking for any listing, a folder that keeps its notes in subfolders is not mistaken for an empty one, and the notes held directly are what matters when choosing where to write.
...
- **RN-AGT-031:** `list_notes` answers the **index** of each note — identifier, name, folder and position, and nothing else — in the defined order, **in pages** of 100 unless the caller asks for up to 500, each continued with the cursor the previous one answered. A page says whether more follow. The cursor is the order key of the last note of the page and not an offset, so a note written or deleted between two pages makes the agent neither read one twice nor skip one.
...
- **RN-AGT-035:** The Notebook Context states, for every folder, the notes it holds directly and, when it has subfolders, the notes its subfolders hold: `(0 notes here, 26 in subfolders)`. A folder without subfolders keeps `(N notes)`.
...
- **RN-AGT-036:** `next_number(notebook, folder)` **issues the next number of a folder** and answers it (RN-KNW-043). It is a write, with a title and hints that say so and that a second call is not the first one again (RN-AGT-009), it shares no tool with a read (RN-AGT-010), and a `VIEWER` is refused like on every write. It lets several agents writing into one folder number what they write without reading the folder first and without colliding.
- **RN-AGT-037:** The connector **keeps, lists and deletes the files of a notebook**. The bytes travel inline, base64, up to a ceiling the refusal names: an agent that must perform an HTTP `PUT` of its own is an agent that cannot keep a file at all. Keeping one answers the reference to write, `![[name]]`, because showing it in a note is what keeping it is for, and `read_note` answers a target that reaches a file as an **attachment** rather than as pending.
- **RN-AGT-038:** `update_note` may carry the same line, and `note_history` answers it, so what a person and an agent leave in the trail reads the same way and in the same place.
- **RN-AGT-039:** A pending link is fine on purpose, as a note still to write, and one that almost reaches a note or a file of the notebook — the same name in another case, without its accents or spacing, or the start of exactly one longer name, word by word — looks broken. `check_notebook` answers both, and the skills end the work with it: every link that looks broken is fixed, and every other pending link is written or named to the person as still to write.
- **RN-AGT-033:** Reading a note answers **the trail of the folder it lives in**, from the root of the notebook down, by name. The trail is where the note is, never what identifies it: no link and no address carries it.
- **RN-AGT-034:** Reading a note answers, beside its body and without changing it, **every link target the note writes and what each resolves to**: the notes it reaches, each with its identifier and folder trail, whether a name or an alias answered, or that it is pending. A target reaching two notes lists both, in no order. The answer comes from the link projection and is as recent as it is; a revision read with `asOf` carries no links, because the projection has no past.
...
- **RN-AGT-026:** Outside production, the connector **declares the environment it serves and the exact version it runs**: the `serverInfo` of the handshake carries that version and a title naming the environment, `MemorySmith.app (staging)`, the `instructions` returned at initialisation say which environment it is and that what is written there is disposable, and `whoami` opens with the same. An agent connected to staging is told so before it writes anything, because nothing in a notebook tells the two environments apart, and a person choosing between two connectors in a client's list is told so by the title. In production neither the environment nor the warning appears, the title is `MemorySmith.app` and the version is the released one; the instructions themselves are sent in every environment (RN-AGT-028).
...
- **RN-AGT-040:** In every environment, the `serverInfo` of the handshake **says who the connector is**, in every field revision 2025-11-25 of the protocol gives it: the name, the title and the version of RN-AGT-026, the slogan of the product as its description, the site of the same environment as its website, and the symbol as icons, PNG and served by that site. A client lists a connector by what the server declares, and one that declares only a name leaves each client to make up the rest — one showed the symbol on a white plate, the other a placeholder of its own. The connector answers in the revision a client asks for when it speaks it, and in the newest otherwise, because a client may disconnect from a revision it does not support.
...
Curation answers knowledge management questions: how much of the content is mature, how much has been through human review, how the notes spread across types and tags. The answer comes from the third projection, the **facets**: aggregatable frontmatter attributes, extracted from each note the moment it changes and kept as counts per notebook. The set of attributes is not fixed: whoever defines the frontmatter of the notes is the Guidance of each notebook, and the projection discovers it by the shape of the values.

**Where they are read is the graph of the notebook**, which colours and filters by them, and the search, which resolves an unknown query prefix as a facet (RN-DSC-026). They used to also be charted on the dashboard, over every notebook at once; that answered a question nobody asks at the door, and cost a second read of every notebook to draw. The first screen now says which notebooks there are and what fills the space of the subscription, and the facets are read where a notebook is.

- **RN-DSC-017:** Curation is served by a derived projection, fed by note events. No screen and no tool scans notes to count; the one that counts is the projector, once, at the moment of the change.
...
- **RN-DSC-021:** A note without a facet counts as a missing value; it is never rejected nor corrected. The projection describes the notebook as it is, and pointing out a gap is the job of whatever reads it, not of the writer.
...
- **RN-DSC-046:** A wikilink is drawn by **what its target reaches**: a note it reaches by name or by a spelling that note declares is a link, and only a target that reaches **nothing** is drawn as pending. The two are not the same question, and asking the wrong one is what drew a link an alias answered as a link to nothing — and left an embed of one expanding nothing — while clicking it reached the note. What tells a name from an alias belongs where the reader is choosing, which is the dialog below. A target that does not resolve to exactly one note **offers the choice in a dialog over the page**, opened by the click, on every reading surface — the note, the folder, the Guidance and the Templates. Over the page and not in place: a reading surface holds containers that scroll on their own, a table first among them, and a choice drawn inside one of them is clipped by it. The dialog says that the link reaches more than one note and what tells them apart: it lists every note the target reaches, each written as its folder trail from the root and its name, which is unique once a folder holds one note of each name (RN-KNW-042), and says whether the target matched a name or an alias — because the two are not equally durable (RN-DSC-053). What matched decides what leads each option: a name matched puts the folder trail first, since every option carries that same name; an alias matched puts the name first. It closes with `Esc`, with the backdrop and with its own controls, and the focus returns to the link. The link keeps a real address, `/notebooks/:notebookId/links/`, for an address that is pasted, bookmarked or opened in a new tab, and that page renders the same explanation and the same options, from one component. It is notebook-wide, because resolution is.
...
- **RN-DSC-061:** A reading surface tells a file from a note by **what the notebook holds** and never by the shape of the target: a target reaches a note, a file, or nothing, and only the third is drawn as pending. **The type decides how a file is drawn** — an image, audio and video are drawn, because a browser draws them without executing anything, and everything else, the PDF included, is a card with what it is and two verbs, Open and Download. `image/svg+xml` is drawn as an image and never inlined, which is the boundary §7.9 draws for raw HTML applied to bytes. The bytes are served through a link minted where the file renders, pointing at the object store and not at the API, so a file somebody uploaded is never served from the origin the product runs in.
- **RN-DSC-062:** A footnote reference is drawn as a link to its note, gathered at the end of the note and numbered by the order of first reference, with a link back; **following either moves within the note** and never opens another page, and what heads the notes and labels the way back is in the locale of the reader (markdown-spec §7.12).
- **RN-DSC-063:** A footnote is **display only**: neither `[^label]` nor its definition produces an edge, an attribute or an index entry, and a line beginning `[^label]:` is never read as a link reference definition. What a definition contains is read like any other text, so a wikilink written inside one is an edge (markdown-spec §7.12, §3.8).
...
- **RN-DSC-059:** *(removed in 0.6.0)* Reserved for drawing the folder where a note is cited, a scope dropped before it was built, because the note page already draws the trail in its breadcrumb and reading a note now answers it (RN-AGT-033). The number is preserved and will never be reused.
- **RN-DSC-058:** The **title of the tab** names what the page shows, then the notebook that holds it, then the product: the name of the note, of the folder or of the link target, or the page of the notebook — Guidance, Templates, the folders, the graph — and the notebook alone on its context. A note with no name is titled with the label every other surface gives it (RN-KNW-036). Since no address carries a name, the title is what tells two tabs, two entries of the history and two bookmarks apart.
...
- **RN-DSC-048:** A `|` in the alt text of an image separates the **description** from the **dimensions**: what precedes it is the description and is never dropped, what follows it is width, or width and height, in CSS pixels, and is never rendered as text. A value that is neither form stays part of the description, because deleting what an author wrote into an accessibility label is the worse of the two failures. **The embed of an attachment carries the same thing in the same pipe** — `![[engelbart.jpg|100x145]]` — and it is applied to what has a dimension to apply it to: an image and a video have one, an audio player does not and a card is text. There too a value that is not a dimension applies nothing, and there the description is the one the file was kept with (§5.8, #173). **Two numbers mean both**, deformation included, because that is what an author who wrote them asked for; a width on its own keeps the ratio; and a width larger than the column it is drawn in is capped by it, since an image wider than the page is a worse answer than an image smaller than the number.
...
- **RN-AUD-001:** **No path alters an event already recorded** — of application, of operations or of administration — and exactly ONE principal removes one: the purge, and only as part of a deletion (RN-AUD-011). Until 0.6.0 the rule said nothing was ever removed, which made the trail of a notebook outlive everything the notebook was.
...
- **RN-AUD-010:** The trail records the **purge of each unit**, under the authorship of whoever deleted the unit above it, and keeps every earlier event of a unit **whose notebook is still there**. What the purge destroyed is named by the `ContentRef` its event carries, which from that moment points at content that no longer exists. Editing a note destroys nothing: its superseded revisions stay, and only the history of something somebody deleted stops being readable. When the notebook itself is what was deleted, its trail goes with it and one entry says so (RN-AUD-011).
- **RN-AUD-011:** **Deleting a notebook takes the trail of what happened inside it**, and the subscription keeps its life: that it was created, renamed, deleted and destroyed, and who could reach it. Everything scoped inside — the writes of its notes, its Templates, its Guidance, and the purge entry of each of them — is removed by the purge, the one principal that may remove an entry (RN-AUD-001). It is **not retroactive**: a notebook deleted before 0.6.0 keeps the trail it has. The erase alone cannot finish the job, because the events of a purge reach the trail after the purge that wrote them has ended, so the worker also **closes** the trail of that notebook and nothing of it is appended again. The life of the notebook is appended either way. A notebook still alive is untouched: removing a folder or a note closes nothing.
- **RN-AUD-012:** A write may carry **a line from its author about the change**, recorded in the trail beside the instant and the authorship, immutable like every entry, and answered by the history of the note. It is **optional**: a write with no line is recorded without one, which is what every write has been until now. The history of a note is readable in the interface, where each entry says when it happened, who wrote it and what they said about it.
...
- **RN-PRT-012:** **An import always creates a new notebook** and never writes into an existing one. There is no merge, no conflict and no question of what wins. The name of the new notebook is **given by whoever imports**, because a subscription holds each notebook name once (RN-KNW-032) and a document often comes home to the subscription it left: importing the same document twice gives two notebooks **under another name**, and never a suffix the server invented. A name already taken is said before anything is uploaded, and the server remains the judge of one taken meanwhile. An import that fails or is cancelled keeps nothing: what it had started is taken back down whole (RN-PRT-018).
...
- **RN-PRT-015:** An import reports what it **actually wrote under the selection**: the notebook it created, its Guidance when that was imported, and how many folders, Templates and notes.
- **RN-PRT-016:** The export carries **the last number each folder issued** (RN-KNW-043), and the import restores it on the folder it creates, so a notebook brought back never issues a number its notes already carry. A document without it reads as a folder that never issued one.
- **RN-PRT-017:** An import may write **part of a document**: the Guidance, the history, each folder, each Template and each note are selected on their own, by the identifiers the document carries. A Template carries the folder it belongs to, as a path when nothing else of that folder was chosen. A folder that is **not** selected but holds something that is is written **as a path** — its name and its description, and nothing else of its own — so a note never arrives without the folder it lives in. The last number each folder issued is restored whatever was selected (RN-PRT-016), so a number is never issued twice. Two notes of one name in one folder refuse the import **only when both are selected** (RN-KNW-042), and the conflict is resolved by leaving one out rather than by editing the file. A notebook is often wanted for its design — its Guidance, its folders and their Templates — rather than for its notes, and the only way used to be importing everything and deleting by hand. The interface asks it in one chooser, whose first tab holds the question and the answer to it: **the whole notebook** — the context, the notes and the history — or part of it, which **asks the scope before the items**: five species, the Guidance, the history, the folders, the Templates and the notes, each turned on or off on its own and each saying how much of it there is. Under the whole notebook those five rows are still drawn, stating what travels and how much of it there is instead of asking. The three that have a hierarchy are then either whole or chosen in a tab of their own, and each of those tabs offers only what the folders let through, because a Template and a note belong to a folder and neither travels without one — which the screen states where it shows them, rather than leaving a note tickable under a folder that was let go. It used to ask item by item, in one tree that mixed the five, so the first question anybody has — the design of the notebook, or the notebook — had nowhere to be answered, and the sizes of what was on offer appeared only after every box had been ticked. `structure only` was a third preset and is gone, because the Guidance and each Template are species of the scope and the preset was a selection anybody can make in one click.
- **RN-PRT-018:** An import is a **job with a status**: it is started, it reports its progress in notes written, and it ends as imported, failed or cancelled. Cancelling, and failing at any write, **take the notebook back down whole**, which frees its name (RN-PRT-014, RN-KNW-033). It used to run inside the request that asked for it, and the function serving that request stops at 29 seconds, so a notebook large enough could not be imported at all — and the gateway answered without CORS headers, so the browser said only `Failed to fetch`. A refusal reaches whoever asked as a **code**, not as a sentence, because what shows it speaks the language of the person.
- **RN-PRT-019:** An export is a **job with a status**: it is started, it reports its progress in notes read, and it ends as ready or failed. It used to run inside the request that asked for it, and the function serving that request stops at 29 seconds, so a notebook large enough could not be exported at all and the interface said only that it had failed. Its archive is written under the subscription prefix like everything else, and reached through a link **issued at the moment of each download**, never through a link that was stored: a stored link is one that has expired by the time somebody comes back to it.
- **RN-PRT-020:** An export is **kept until whoever generated it deletes it**, and deleting it is definitive. It is seen, downloaded and deleted **only by whoever generated it**, because other members of the subscription may see less of that notebook and a listing must never reveal a notebook somebody cannot see (rule 9 of the design). It **survives the deletion of its notebook**: it is a document and not a part of the notebook, and bringing it back is an import, which mints every identifier anew (RN-PRT-013). **It is imported from where it is kept**, without being downloaded and uploaded again: the archive is copied where the product keeps it, and the export is not consumed — it stays kept, with its space, and may be imported again. Only whoever generated it can import it this way, and only once it is ready. It counts towards the storage of the subscription while it is kept (RN-SUB-021). Generating and deleting one carry the authorship of whoever did it. *(Up to 0.6.0 the archive was answered as a link of fifteen minutes that nothing listed, and the bucket destroyed it the next day: a backup nobody could come back to.)*
- **RN-PRT-022:** An export may be asked to carry the **history of the notebook**: every entry of the trail about it, and **the content of every revision those entries name**. A history whose content cannot be read says that something was written and never what, which is the state a purged notebook leaves behind and the reason this carries both. Each revision is reached by the exact pair its entry carries and never by listing the versions of an object, which belongs to the purge alone (rule 8 of the design). It is off unless it is asked for, the archive is larger for it, and a kept export counts towards the storage of the subscription like any other (RN-SUB-021). Since deleting a notebook takes its trail with it (RN-AUD-011), this is the one way a history survives on purpose. **Under a selection it carries the history of what the archive carries**, and only the revisions those entries name: an entry about a folder or a note left out is left out with it, an entry about the Guidance travels only when the Guidance does, and one about a Template only when that Template does. The entries about the life of the notebook always travel, because an import reads the old identifier of the notebook off them. **Without a selection it carries the trail whole**, the entries about notes that were deleted included: a deleted note travels nowhere else (RN-PRT-006), so the whole notebook is the one archive that keeps them. The filter is applied before a revision is fetched, so nothing is read from the object store for an entry the archive will not hold.
- **RN-PRT-024:** An export may write **part of a notebook**, by the same shape a selection takes on the way in (RN-PRT-017): the Guidance, the Template of each folder, each note, the history and **the files** chosen on their own, a folder that holds something chosen carried as a path, and a Template carrying the folder it belongs to. The files are the one species nothing above can take out, because no folder holds one (RN-PRT-025). **No selection is the whole notebook**, and the whole notebook carries its history whole (RN-PRT-022), while a selection carries the history of what it carries. What the two sides choose from differs and cannot not differ: an import reads its document in the browser before a byte is uploaded, and an export has no document yet, so it chooses from the structure the API answers.
- **RN-PRT-025:** An export carries **the files the notebook keeps**, with their bytes, their name, description, type, tags and path, and an import brings them back through the same door an upload uses — so a file whose bytes do not support its declared type is refused there as it would be at the door, and a name the notebook already holds is refused too. They are a **species of the selection**, chosen whole or **by name**, item by item — by name on both sides, because no identifier of a file travels and a name is the only address the two halves of a transfer share. **Every file is the default**, and the reason is the one that used to keep them out of the selection entirely: a file belongs to the notebook rather than to a folder, any note carried may reference any of them, and an embed that lands pending because the picture was left behind is a worse default than a larger archive. What changed is that the default is now **said** — a screen that asks what travels and never names the megabytes it is about to carry is asking half a question — and that it can be answered otherwise (#176). No identifier travels: an import mints its own, and a note reaches a file by **name**, which is what makes an imported `![[name]]` find the file it always found. The document format goes from `1.1` to `1.2`, and an archive of any of the three still imports.
- **RN-PRT-023:** An import **reproduces a carried history as entries of this subscription**, keeping the person and the instant each entry was written with. Every identifier is re-keyed onto what the import minted (RN-PRT-013), and an entry about something the selection left out is left out with it. Each revision is stored as content of this subscription first, and its entry is pointed at that, which is what makes `read_note(asOf)` answer on an imported notebook. An entry whose body did not travel keeps saying who wrote and when and points at nothing. **Every reproduced entry records the transfer that brought it in**, because an archive is a file anybody can craft: the trail can always answer where a line came from. Each one is validated against the contract of its type before it is written, exactly as an event of the bus is.
- **RN-PRT-021:** Deleting a notebook that has kept exports **says so where the deletion is confirmed**, with how many there are and that they stay. A notebook is deleted through the connector, so that confirmation is the answer of the tool.
- **RN-PRT-026:** A transfer that ended is **deleted by whoever started it**, from its row. Deleting an export destroys its bytes and gives back their space (RN-PRT-020); deleting an import removes **its record alone**, because it keeps no bytes of its own and the notebook it created is a notebook like any other. A transfer still running is **refused**: an import is cancelled first, which takes it back down whole (RN-PRT-018), and nothing with a worker behind it loses its record under the worker.
...
| Screen | Content |
|---|---|
| Notebook catalogue | Cards in the order of the names, compared in the language of the reader, each with a drawing that is decoration and never the graph of the notebook, the name, the description, the note count and the last update. Under them the space of the subscription: the total against the quota, what fills it by kind, which notebook holds it, and how many notebooks, folders and revisions there are (RN-SUB-024). The cards never wait for the space, and when the split cannot be read the total the session knows is still shown. Behind the `⋯` of a card, *Export notebook* opens the export with that notebook chosen, and *Delete notebook…* — offered only where the role the API answers for that notebook is the owner's (RN-KNW-033) — asks on the card itself, with the consequence and a way to export first, and deletes on *Delete for good*; a notebook still being purged leaves the list of the space and is stated as what it still holds |
...
| Transfers → a new transfer | **One dialog, whichever way it goes.** An export asks which notebook, among the ones the person can see; an import takes a file from the device or one of the exports the person keeps, and the **browser reads it on the spot** — unzipping it and validating the document against the same format the server reads — so what it carries, and the name it arrives under, are known before a byte is uploaded, and the name is checked against the notebooks of the subscription as it is typed. Both then ask what travels, in the chooser the two sides share, which is on the screen whichever the answer is: its first tab asks the whole notebook or part of it, and a species chosen item by item opens a tab of its own. The summary states what will be created and how many links will point at notes left out, and **what refuses the import is a tab of its own** — a name the subscription already holds, and a name carried twice in one folder — each row saying what refuses and the way out of it, with one line in the foot that does not grow with them. Starting one closes the dialog, because what happens next is followed in Transfers, and a refusal reads in the language of the interface with nothing kept (RN-PRT-015, RN-PRT-017, RN-PRT-018, RN-PRT-024) |
| Transfers | Every export and every import of whoever is signed in, with what each one is doing or what it produced: an icon beside the user menu, in the place a browser puts its downloads and never as a notification bell, and a page with the history. An export is downloaded again from here, through a link issued at that moment, imported from here as a new notebook without passing through the device, and deleted from here, which destroys its bytes; an import that ended is deleted from here too, which removes its row and leaves the notebook it created (RN-PRT-026). A running import is cancelled from here, which takes it back down whole (RN-PRT-018), and a transfer that failed says why. The space the kept exports occupy is stated at the top, because it counts towards the quota (RN-PRT-019, RN-PRT-020, RN-SUB-021) |

**What the interface does not reach.** The governance of the subscription, that is members, roles, ownership, notebook ceilings and switching the active subscription, and the platform area of §4.6, exist in the API and have no screen. So do the audit and health reads other than the history of a note, that is notebook activity, and pending links and orphan notes as lists. Whoever needs them today calls the API or uses the operations scripts, and what is missing is recorded in the issues of the repository, which is where the future lives.
...
- **Every screen of a notebook opens with the same bar, and its trail names the page.** The bar stays fixed above the content while it scrolls; the trail starts at the notebook and ends at the page that is open, so no heading repeats it. When it does not fit it gives up space in one order: the name of the notebook shortens to a minimum, then the folders in the middle collapse into one *…* that leads to the deepest of them, and only then does the last one shorten — where the person is never leaves the bar. The Guidance, the Templates and the page of the folders sit under the Notebook Context in it. **The note is the exception**: its trail stops at the folder that holds it, because the name of the note is content of the notebook — it is what the copied Markdown carries — and is the title of the page. Only the note puts actions in the bar: copying its Markdown, opening its history — the modal of the Controles on a computer and a sheet on a phone, read only when it is opened — and, for whoever may write, editing it. **On a phone the sidebar is a sheet** whose start is always showing at the foot of the screen: raised, it holds a bar of icons for the search, the Context, its three parts and the graph, and below it the tree, and it closes when a page is opened.
