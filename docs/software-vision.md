...
| Screen | Content |
|---|---|
| Notebook catalogue | Cards in the order of the names, compared in the language of the reader, each with a drawing that is decoration and never the graph of the notebook, the name, the description, the note count and the last update. Under them the space of the subscription: the total against the quota, what fills it by kind, which notebook holds it, and how many notebooks, folders and revisions there are (RN-SUB-024). The cards never wait for the space, and when the split cannot be read the total the session knows is still shown |
