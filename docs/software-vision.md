...
2. **Curation.** People review what came in and drive the corrections, and the interface is where they read: the note and the structure as the agent receives them, the distribution of the notebook on its graph, which colours and filters by the facets (§10.3), and, across every notebook at once, how much there is and how much of it is broken — the count of pending links and of orphan notes on the dashboard (§13).
...
| Screen | Content |
|---|---|
| Notebook catalogue | Cards with the name, the description, the note count and the last update, and under them four numbers about everything the account holds: how many notebooks, how many notes, how many pending links and how many orphan notes. A notebook that did not answer is named, because four numbers that quietly under-report are worse than four missing |
