...
1. **Ingestão.** O agente lê um corpo de material — normas, legislação, documentação, pesquisa — e escreve esse conhecimento como notas no vault, obedecendo ao Guidance (o que este vault é), à estrutura de pastas (onde cada coisa vai) e ao Template da pasta (como a nota se estrutura).
...
| Trabalho regulado (auditoria, jurídico, compliance) | O parecer emitido não pode ser demonstrado com a base de ontem | Histórico por revisão, autoria humano + agente, trilha imutável |
...
| **PP10** | **Erro é interface** | Toda recusa devolvida ao agente precisa dizer o que fazer em seguida — argumento faltante devolve as opções válidas, conflito devolve o conteúdo atual |
...
- **A assinatura ativa é escolhida, não deduzida.** A sessão age em nome de uma assinatura por vez; trocar é ação explícita.
...
O que ele vê é metadado de assinatura: titular, e-mail, status, datas, contagem de workspaces. Nunca nome de vault, nunca conteúdo de nota.
...
**A UI esconde o nível de workspace enquanto houver um só, e esconde a troca de assinatura enquanto houver um só vínculo.** O modelo é completo desde o começo; a interface é que aparece por etapas.
...
- **RN-SUB-003:** Nenhuma consulta pode retornar dados de mais de uma assinatura. Duas perguntas atravessam a fronteira e são as únicas: *"de quais assinaturas este usuário participa?"* e a listagem administrativa de assinaturas por status — e nenhuma das duas revela conteúdo.
...
**Transferência.** O `OWNER` pode transferir a titularidade para outro membro da assinatura; a partir daí ele passa a ser `EDITOR`. A assinatura nunca fica sem titular — a transferência é atômica, não é "remover e depois nomear".
...
| `VIEWER` | `EDITOR` | **recusado** — o teto não promove |

Só existe um valor de teto: `VIEWER`. Não há "sem acesso": **quem é membro do workspace enxerga todos os vaults dele**; o que o teto controla é escrever, não ver. Tirar um vault do alcance de alguém continua sendo motivo para um workspace separado — o que mantém a pergunta "quem vê o quê" respondível olhando só a lista de membros.

O teto não se aplica ao `OWNER`: ele é titular da assinatura e alcança tudo.

### 5.4 Regras de negócio — Access
...
- **RN-ACC-010:** `VIEWER` — por papel de workspace ou por teto de vault — recebe recusa em qualquer operação de escrita, tanto pela UI quanto pelo MCP.
...
**Knowledge é o core domain**: é onde está a regra que nenhum concorrente resolve de graça — estrutura declarada, ordem significativa, papéis de conteúdo e escrita concorrente barata. Todo o resto existe para servi-lo ou para transportá-lo.
...
name, slug,
description,                 -- OBRIGATÓRIA, 1–500 caracteres: é ela que orienta o agente
...
- **RN-KNW-020:** O `slug` da nota é único **dentro do vault**, não dentro da pasta — é assim que os links resolvem (§10.1).
...
**O MCP é o contrato público do produto.** A API interna (`architecture-guide.md` §12) existe para servir a UI; é o catálogo de tools abaixo que os clientes externos consomem, e é ele que a política de versionamento protege (`CLAUDE.md` § Versioning Policy).
...
| **`get_vault_context`** | `(vault)` | **A chamada principal.** Guidance integral + árvore com descrições, ordem, contagem de notas e quais pastas têm template |
...
A saída de `get_vault_context` é **o produto**, não um detalhe de apresentação: é o equivalente exato do que o agente obtém hoje lendo o documento de orientação e rodando `ls -R` na pasta local — em uma chamada.
...
- **A ordem é a ordem definida**, numerada — é sinal, não enfeite (PP9).
...
- **RN-AGT-004:** `create_note` com um slug já existente no vault devolve `ALREADY_EXISTS` **com o identificador da nota existente**, e nunca cria uma segunda nota. O servidor jamais gera sufixo automático — é isso que transformaria um retry de transporte em duplicata silenciosa.
...
- **RN-DSC-001:** Segmentos de caminho no link (`../normas/`) são deliberadamente ignorados na resolução. A aresta é entre notas, não entre pastas — honrar o caminho faria o link quebrar quando a nota mudasse de pasta, que é exatamente o que o produto existe para evitar.
...
- **RN-DSC-011:** Cada trecho é enriquecido com o contexto de onde veio — vault, pasta, descrição da pasta e título da nota — antes de ser vetorizado. A descrição da pasta, escrita para orientar o agente, vira sinal de recuperação de graça.
...
- **RN-DSC-013:** Apagar uma nota remove seus vetores imediatamente, inclusive no soft delete. O que sai da listagem sai da busca — conteúdo apagado que continua sendo devolvido é problema de privacidade, não de qualidade.
...
O humano é sempre identificado — mesmo quando quem grava é o agente, a autorização pertence a quem conectou. É esse par que transforma *"escrito por Fulano"* em *"escrito por tal agente, em nome de Fulano, em 12/03"*: a diferença entre um registro e um registro defensável.
...
- **RN-AUD-005:** `read_note(asOf)` devolve o conteúdo vigente na data informada, reconstruído a partir da trilha — permitindo refazer um trabalho lendo a base como ela estava na data de emissão.
- **RN-AUD-006:** Apagar uma nota nunca destrói o conteúdo armazenado; o histórico continua legível.
- **RN-AUD-007:** Destruir conteúdo (**expurgo**) é ato administrativo restrito ao `OWNER`, exige motivo obrigatório e gera evento próprio. A trilha passa a registrar *"aqui houve apagamento, por este motivo, autorizado por esta pessoa"* — uma lacuna silenciosa não é resposta aceitável a um regulador.
...
- **RN-PRT-002:** A ordem de pastas e de notas é codificada como prefixo numérico no nome — é a única forma de preservá-la num sistema de arquivos, que não tem ordem própria.
...
| Nota → Histórico | Linha do tempo com autoria (humano + agente) e diferença entre revisões |
...
| Configurações da assinatura | Titularidade, transferência, retenção legal, expurgo, troca de assinatura ativa — só aparecem quando aplicáveis (PP8) |
...
- Uma assinatura fora de `trial`/`active` leva o usuário à tela de estado, não a uma tela de conteúdo vazia — a diferença entre "não há nada aqui" e "seu acesso está suspenso" é a diferença entre um bug aparente e uma informação.
- Toda operação destrutiva ou de efeito não óbvio — mover nota entre vaults, remover pasta com conteúdo, transferir titularidade, ativar retenção legal, expurgar — mostra a consequência **antes** de confirmar, com número concreto quando houver.
- Onde o papel efetivo difere do papel no workspace por causa de um teto de vault (§5.3), a UI mostra o efetivo e explica a origem — um `EDITOR` que não consegue escrever precisa saber por quê.
...
| Profundidade da travessia de grafo | 3 · teto de 200 nós |
...
- **A aprovação de assinatura entra.** Sem billing, ela é o único portão de entrada: sem ela ninguém acessa nada, e a 0.1.0 não teria como ser usada por um primeiro cliente real. O que fica de fora é o resto do ciclo de vida — suspender, reativar, transferir.

**O que a 0.1.0 assume, e que precisa estar declarado:** a assinatura tem um `OWNER` e nenhum outro membro. Colaboração inteira — convites, `EDITOR`, `VIEWER`, teto por vault — fica para a 0.3.0. O modelo de papéis já existe no domínio desde a primeira linha (PP8); o que não existe ainda é a superfície que o exercita.
...
| Q2 | **Identidade visual** — cor primária, logotipo, tom | Depende de decisão de marca ainda não tomada (`CLAUDE.md` § Brand Identity) |
