---
title: MOC - Service Desk
aliases: [MOC Service Desk, Índice Módulo 2, MOC ITIL]
tags: [moc, dominio/service-desk]
type: moc
maturity: evergreen
reviewed: false
author: CAD Discovery
created: 2026-07-10
---

# MOC - Service Desk (Módulo 2)

Mapa de conteúdo do **service desk ITIL** — Ticket, Change, Problem e apoios.

## Processos (por que existe)
- [[Gestão de Incidentes e Requisições (processo)]]
- [[Gestão de Mudanças (processo)]]
- [[Gestão de Problemas (processo)]]

## Regras de negócio
- [[Priorização (urgência × impacto)]]
- [[SLA e níveis de serviço (regra)]]
- [[Validação e aprovação (regra)]]

## Estrutura (como é composto)
- [[CommonITILObject (base de service desk)]]
- [[Ticket]] · [[Change]] · [[Problem]]
- [[Modelo de Atores ITIL]]
- [[SLM, SLA e OLA]]
- [[Categorias e templates ITIL]]

## Comportamento (como funciona)
- [[Ciclo de vida de um Ticket (máquina de estados)]]
- [[Fluxo de followups, tarefas e solução]]

## Views
- [[Máquina de estados do Ticket (view)]]
- [[Modelo de dados do Service Desk (view)]]

## Investigações abertas
- [[INV-1-004 · Ações de escalonamento de SLA]]
- [[INV-1-005 · Regras exatas de transição de status por perfil]]

## Evidências
EV-1-008 a EV-1-014 — ver [[Registro de Evidências]].

## Base herdada
Todo este módulo assenta sobre o [[MOC - Foundation]] (CommonDBTM, direitos, entidades, busca).
