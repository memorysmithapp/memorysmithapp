---
title: MOC - Ativos e Inventário
aliases: [MOC Ativos, Índice Módulo 3, MOC CMDB]
tags: [moc, dominio/ativos]
type: moc
status: confirmed
maturity: evergreen
reviewed: false
author: CAD Discovery
created: 2026-07-10
---

# MOC - Ativos e Inventário (Módulo 3)

Mapa de conteúdo do **CMDB, inventário e DCIM**.

## Processos (por que existe)
- [[Gestão de Ativos e Configuração (SACM)]]
- [[Gestão de Software e Licenças (processo)]]
- [[Inventário automático (processo)]]

## Estrutura (como é composto)
- [[Modelo de Ativos (padrão comum)]]
- [[Composição de um Ativo (componentes)]]
- [[Software, Versões e Licenças]]
- [[Rede (portas, IP, VLAN)]]
- [[DCIM (Datacenter → Rack)]]
- [[Infocom (dados financeiros do ativo)]]
- [[Ativos Customizáveis (AssetDefinition)]]

## Comportamento (como funciona)
- [[Fluxo de inventário nativo]]

## Views
- [[Composição do ativo Computer (view)]]

## Investigações abertas
- [[INV-1-006 · Capacities disponíveis para ativos customizados]]
- [[INV-1-007 · Protocolo e endpoint do agente de inventário]]

## Evidências
EV-1-015 a EV-1-022 — ver [[Registro de Evidências]].

## Base herdada
Assenta sobre [[MOC - Foundation]]; conecta-se ao [[MOC - Service Desk]] (item afetado em
chamados) e ao Módulo 4 (contratos/financeiro via [[Infocom (dados financeiros do ativo)]]).
