...
Toda afirmação normativa em uma nota aponta para o dispositivo (artigo, anexo) da norma original, identificada por número e ano. Uma nota sem `source` verificável não entra no caderno. Em matéria regulatória isso é inegociável: interpretação errada propaga silenciosamente pelo grafo.

## Onde cada nota entra

A árvore de pastas do caderno, com a descrição de cada pasta, é a regra de entrada: antes de escrever, leia a descrição da pasta de destino. Ela diz o que se guarda ali e o que pertence a outra pasta.
