---
title: MOC - Gestão
aliases: [MOC Gestão, Índice Módulo 4, MOC Management]
tags: [moc, dominio/gestao]
type: moc
maturity: evergreen
reviewed: false
author: CAD Discovery
created: 2026-07-10
---

# MOC - Gestão (Módulo 4)

Mapa de conteúdo de **contratos, financeiro, fornecedores, documentos, projetos e reservas**.

## Processos (por que existe)
- [[Gestão de Contratos (processo)]]
- [[Gestão Financeira de TI]]
- [[Gestão de Projetos (processo)]]
- [[Reserva de Ativos e Documentos (processos)]]

## Estrutura (como é composto)
- [[Contratos (Contract)]]
- [[Fornecedores e Contatos]]
- [[Orçamentos e Custos]]
- [[Documentos (Document)]]
- [[Projetos (Project)]]
- [[Reservas e Consumíveis]]

## Views
- [[Modelo de dados de Gestão (view)]]

## Investigações abertas
- [[INV-1-008 · Alertas e crons de vencimento]]

## Evidências
EV-1-023 a EV-1-027 — ver [[Registro de Evidências]].

## Conexões
Ponte com o CMDB via [[Infocom (dados financeiros do ativo)]] ([[MOC - Ativos e Inventário]]);
custos de atendimento vêm do [[MOC - Service Desk]]. Assenta sobre [[MOC - Foundation]].
